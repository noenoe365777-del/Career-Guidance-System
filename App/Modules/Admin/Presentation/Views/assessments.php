...
$pageTitle = $pageTitle ?? 'Assessment Management';
?><!doctype html>
                ...
                <div class="max-w-[1400px] mx-auto space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-3">
                        <div class="glass-card rounded-2xl p-6">
                            <p class="text-sm font-semibold uppercase tracking-wide text-slate-400">Total Assessments</p>
                            <div class="mt-3 text-3xl font-bold text-slate-800"><?= (int)$assessmentCount ?></div>
                        </div>
                        <div class="glass-card rounded-2xl p-6">
                            <p class="text-sm font-semibold uppercase tracking-wide text-slate-400">Active</p>
                            <div class="mt-3 text-3xl font-bold text-emerald-600"><?= (int)array_filter($assessments, static fn($item) => strtolower((string)($item['status'] ?? 'active')) === 'active') ?></div>
                        </div>
                        <div class="glass-card rounded-2xl p-6">
                    ...
                    <div class="glass-card rounded-3xl border border-slate-100 bg-white overflow-hidden">
                        <div class="border-b border-slate-100 px-6 py-4">
                            <h3 class="text-lg font-semibold text-slate-800">Assessment List</h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-100 text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    <?php if ($assessments === []): ?>
                                        <tr><td colspan="3" class="px-6 py-8 text-center text-slate-400">No assessment records available yet.</td></tr>
                                    <?php else: ?>
                                        <?php foreach ($assessments as $assessment): ?>
                                            <tr>
                                                <td class="px-6 py-4 font-semibold text-slate-700"><?= htmlspecialchars((string)($assessment['title'] ?? 'Untitled Assessment')) ?></td>
                                                <td class="px-6 py-4 text-slate-500"><?= htmlspecialchars((string)($assessment['assessment_type'] ?? 'general')) ?></td>
                                                <td class="px-6 py-4"><span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600"><?= htmlspecialchars((string)($assessment['status'] ?? 'active')) ?></span></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tbody>
