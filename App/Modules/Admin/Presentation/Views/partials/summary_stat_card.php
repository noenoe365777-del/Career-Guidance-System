...
if (!function_exists('renderAdminSummaryCard')) {
    function renderAdminSummaryCard(array $config): void
    {
        ...
        $filter = (string)($config['filter'] ?? 'all');
        ...
        $onclick = (string)($config['onclick'] ?? '');
        $type = (string)($config['type'] ?? 'button');
        $buttonType = $type === 'link' ? 'a' : 'button';
        $buttonAttrs = '';
        ...
        if ($onclick !== '') {
            $buttonAttrs .= ' onclick="' . htmlspecialchars($onclick, ENT_QUOTES, 'UTF-8') . '"';
        }

        $classes = ['stat-card', 'card-in', $delayClass, $active ? 'active' : '', $extraClass];
        $classAttr = implode(' ', array_filter($classes));
        $valueText = $value === '' ? '0' : $value;
        $dataValue = $valueNumber > 0 ? $valueNumber : 0;
        $dataSuffix = $suffix;
        $cardStyle = 'background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:24px;box-shadow:0 6px 18px rgba(15,23,42,0.04);cursor:pointer;text-align:left;transition:transform 0.3s ease-out, box-shadow 0.3s ease-out, border-color 0.3s ease-out, background-color 0.3s ease-out;';
        ...
        echo '<' . $buttonType . ' class="' . htmlspecialchars($classAttr, ENT_QUOTES, 'UTF-8') . '" ' . $buttonAttrs . ' data-filter="' . htmlspecialchars($filter, ENT_QUOTES, 'UTF-8') . '" data-value="' . (int)$dataValue . '" data-suffix="' . htmlspecialchars($dataSuffix, ENT_QUOTES, 'UTF-8') . '" data-counter-id="' . htmlspecialchars($counterId, ENT_QUOTES, 'UTF-8') . '" style="' . htmlspecialchars($cardStyle, ENT_QUOTES, 'UTF-8') . '">';
