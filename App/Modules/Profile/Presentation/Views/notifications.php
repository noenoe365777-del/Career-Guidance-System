...
?>
<style>
:root { --primary: #3B82F6; --primary-light: #60A5FA; --primary-dark: #2563EB; --bg-page: #F8FAFC; --radius: 16px; }
...
.chip-active { background:#3B82F6 !important; color:#fff !important; box-shadow:0 2px 8px rgba(59,130,246,0.3); }
...
<div class="animate-in flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Stay updated with your assessments and career journey.</p>
    </div>
    <div class="flex items-center gap-2">
        <button id="markAllReadBtn"
                class="inline-flex items-center gap-1.5 rounded-xl bg-blue-50 px-3.5 py-2 text-xs font-semibold text-blue-600 transition-all duration-200 hover:bg-blue-100 active:scale-[0.97] <?= $unreadCount === 0 ? 'opacity-50 pointer-events-none' : '' ?>">
...
<div class="animate-in d1 rounded-[var(--radius)] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden">
        ...
        <form method="get" class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="relative flex-1 min-w-0">
                ...
                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                       ...
                       class="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-4 text-sm text-slate-700 outline-none transition-all duration-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100">
            </div>
            <?php if ($search !== '' || $type !== ''): ?>
            <a href="<?= BASE_URL ?>/index.php?page=notifications"
               class="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-medium text-slate-600 transition-all duration-200 hover:bg-slate-50 no-underline">Clear</a>
            <?php endif; ?>
        </form>
    </div>
    ...
    <div class="px-4 sm:px-5 py-3 border-b border-slate-100 flex flex-wrap items-center gap-2">
        ...
        $chips = [
            ...
            'career' => ['label' => 'Career Maps', 'icon' => 'bi-briefcase'],
            'profile' => ['label' => 'Profile', 'icon' => 'bi-person-check'],
            'system' => ['label' => 'System', 'icon' => 'bi-megaphone'],
        ];
        foreach ($chips as $key => $chip):
            $isActive = ($key === 'all' && $tab === 'all' && $type === '') || ($key === 'unread' && $tab === 'unread') || ($key !== 'all' && $key !== 'unread' && $type === $key);
            $href = $key === 'all' ? BASE_URL . '/index.php?page=notifications' : ($key === 'unread' ? BASE_URL . '/index.php?page=notifications&tab=unread' : BASE_URL . '/index.php?page=notifications&type=' . urlencode($key));
            $icon = $chip['icon'] ?? null;
        ?>
        <a href="<?= $href ?>"
    ...
    <div>
        ...
        <div class="divide-y divide-slate-50">
            <?php foreach ($groups[$gk] as $n):
                ...
                $typeIconMap = [
                    'assessment' => ['icon' => 'bi-clipboard-check', 'bg' => 'bg-blue-50', 'color' => 'text-blue-600'],
                    'career' => ['icon' => 'bi-briefcase', 'bg' => 'bg-amber-50', 'color' => 'text-amber-600'],
                    'profile' => ['icon' => 'bi-person-check', 'bg' => 'bg-emerald-50', 'color' => 'text-emerald-600'],
                    'system' => ['icon' => 'bi-megaphone', 'bg' => 'bg-purple-50', 'color' => 'text-purple-600'],
                ];
                $iconInfo = $typeIconMap[$nType] ?? ['icon' => 'bi-bell', 'bg' => 'bg-slate-50', 'color' => 'text-slate-500'];
            ?>
            <div class="notification-item transition-all duration-150 <?= $nIsRead ? 'bg-white' : 'bg-blue-50/30' ?> hover:bg-slate-50 <?= $nIsRead ? '' : 'border-l-4 border-l-blue-500' ?>" data-id="<?= $nid ?>">
                <div class="flex items-start gap-3.5 px-5 py-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="mt-2 flex items-center gap-3">
                            <?php if ($nLink): ?>
                            <a href="<?= BASE_URL ?>/index.php?page=<?= htmlspecialchars(ltrim($nLink, '/')) ?>" class="text-xs font-medium text-blue-600 hover:text-blue-700 no-underline flex items-center gap-1"><i class="bi bi-box-arrow-up-right"></i> View</a>
                            <?php endif; ?>
                            <?php if (!$nIsRead): ?>
    ...
    <div class="flex items-center justify-between border-t border-slate-100 px-5 py-3">
        ...
        <div class="flex items-center gap-1">
            ...
            <a href="<?= BASE_URL ?>/index.php?page=notifications&tab=<?= urlencode($tab) ?>&p=<?= $currentPage - 1 ?><?= $type ? '&type='.urlencode($type) : '' ?><?= $search ? '&search='.urlencode($search) : '' ?>"
            ...
            <a href="<?= BASE_URL ?>/index.php?page=notifications&tab=<?= urlencode($tab) ?>&p=<?= $i ?><?= $type ? '&type='.urlencode($type) : '' ?><?= $search ? '&search='.urlencode($search) : '' ?>"
               class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-xs font-medium no-underline transition-all duration-150 <?= $i === $currentPage ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100' ?>">
                <?= $i ?>
            </a>
            <?php endfor; ?>
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target.classList.contains('mark-read-btn')) {
            ...
            fetch(baseUrl + 'api-mark-read', {
                ...
                if (data.success) {
                    item.classList.remove('bg-blue-50/30', 'border-l-4', 'border-l-blue-500');
                    item.classList.add('bg-white');
                    var title = item.querySelector('.font-bold');
                    if (title) title.className = 'text-sm font-medium text-slate-700';
                    target.remove();
                    setBadge(data.unread_count);
                    var unreadItems = document.querySelectorAll('.notification-item.border-l-blue-500').length;
                    var markAllBtn = document.getElementById('markAllReadBtn');
                    if (markAllBtn && unreadItems === 0) {
                        markAllBtn.classList.add('opacity-50', 'pointer-events-none');
                    }
                }
            });
        }
    ...
    if (markAllBtn) {
        markAllBtn.addEventListener('click', function() {
            fetch(baseUrl + 'api-mark-all-read', {
                ...
                if (data.success) {
                    document.querySelectorAll('.notification-item.bg-blue-50\\/30').forEach(function(item) {
                        item.classList.remove('bg-blue-50/30', 'border-l-4', 'border-l-blue-500');
                        item.classList.add('bg-white');
                    });
                    ...
                    document.querySelectorAll('.notification-item .font-bold').forEach(function(title) {
                        if (title) title.className = 'text-sm font-medium text-slate-700';
                    });
                    setBadge(0);
                    markAllBtn.classList.add('opacity-50', 'pointer-events-none');
