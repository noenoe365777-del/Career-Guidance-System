...
<section class="min-h-[60vh] flex items-center justify-center py-20">
    <div class="max-w-lg w-full bg-white rounded-2xl p-8 shadow">
        ...
        <form action="<?= BASE_URL ?>/index.php?page=verify-reset-code" method="post" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-slate-700">Registered Email</label>
                <input type="email" name="email" required value="<?= htmlspecialchars($old['email'] ?? $pendingEmail) ?>"
            ...
            <div class="flex items-center justify-between">
                <button type="submit" class="px-6 py-3 bg-indigo-600 text-white rounded-lg font-semibold">Verify Code</button>
