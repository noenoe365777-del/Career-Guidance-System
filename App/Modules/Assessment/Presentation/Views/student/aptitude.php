...
<main class="flex-grow bg-[#f8fafc] w-full">
    <div class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        <a href="<?= BASE_URL ?>/index.php?page=<?= $backToPage ?? 'assessments' ?>"
           class="inline-flex items-center text-sm font-bold transition-colors hover:text-green-700 <?= $accentClass ?? 'text-[#16a34a]' ?>">
        ...
        <div class="mt-5 rounded-xl border border-slate-200/60 bg-white p-6 shadow-sm md:p-8">
            ...
            <p class="mt-2 text-sm font-medium text-slate-500">
                Answer the following <?= $questionCount ?> questions to understand your reasoning and problem-solving strengths.
            </p>
        ...
        <div class="mt-6 rounded-xl border border-slate-200/60 bg-white p-6 shadow-sm md:p-8">
            <h2 class="mb-6 text-sm font-bold uppercase tracking-wider text-[#16a34a]">
                Questions
            </h2>
            ...
            <form method="POST" class="space-y-6">
                <?php foreach ($questions as $index => $question): ?>
                    <div class="space-y-3">
                        <div class="flex items-center gap-3">
                            <div class="flex h-6 w-6 shrink-0 select-none items-center justify-center rounded-full bg-[#16a34a] text-xs font-bold text-white">
                                <?= $index + 1 ?>
                            </div>
                            ...
                            <?php foreach ($question['options'] ?? [] as $opt): ?>
                                <label class="flex cursor-pointer select-none items-center gap-2 rounded-lg px-3 py-2.5 transition hover:bg-slate-50 group">
                                    <input type="radio"
                                           ...
                                           class="h-4 w-4 border-slate-300 text-[#16a34a] focus:ring-[#16a34a]">
                ...
                <div class="flex justify-center pt-6">
                    <button type="submit"
                            class="rounded-xl px-8 py-3 text-sm font-semibold text-white shadow-md transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] <?= $buttonClass ?? 'bg-[#16a34a] hover:bg-green-700' ?>">
                        Submit Assessment
                    </button>
                </div>
            </form>
